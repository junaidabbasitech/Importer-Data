...
async function generateConsigneeProfileWithFallback(prompt, consigneeName, address) {
  const ai = getAIClient();
  const modelsToTry = ["gemini-3.7-flash", "gemini-3.1-flash-lite", "gemini-flash-latest"];
  async function tryGenerate(model, useSearch) {
  ...
  const fallbackData = {
    ...
    companyProfile: {
      ...
      website: cleanDomain ? `https://www.${cleanDomain}.com` : void 0,
      phone: void 0,
      email: void 0,
    ...
    shipmentMetrics: {
      ...
      lastShipmentDate: (/* @__PURE__ */ new Date()).toISOString().split("T")[0],
      lastShipmentDetails: {
...
async function executeProfileConsignee(consigneeName, address, deepSearch) {
...
DO NOT include any markdown backticks or explanation before or after the JSON. Return only the JSON object.`;
  ...
  parsedData.scannedAt = (/* @__PURE__ */ new Date()).toISOString();
  return parsedData;
}
async function executeSearchByCommodity(params) {
  const {
    commodityOrIndustry,
    hsCode,
    minAnnualTeus = 50,
...
Do NOT add text before or after the JSON array. Return AT LEAST 20 consignee objects in the array!`;
  ...
  if (Array.isArray(rawList)) {
    formattedResults = rawList.map(
      (item, idx) => formatConsigneeToSearchResult(item, idx + 1, queryTerm, minTeuVal, originCountry, destinationPort)
    );
  }
...
function formatConsigneeToSearchResult(item, rankNum, queryTerm, minTeus, originFilter, portFilter) {
  ...
  const cleanPhone = cp.phone && !cp.phone.includes("555-0199") && !cp.phone.includes("xxx-xxxx") ? cp.phone : void 0;
  const cleanEmail = cp.email && !cp.email.startsWith("import-logistics@") && !cp.email.includes("importer.com") ? cp.email : void 0;
  const authenticContacts = Array.isArray(cp.keyContacts) ? cp.keyContacts.filter(
    (c) => c.name && !c.name.includes("VP of Supply Chain") && !c.name.includes("Director of Global Supply Chain") && !c.name.includes("Global Logistics & Supply Chain Director")
  ) : [];
  ...
  const fullProfile = {
    ...
    companyProfile: {
      ...
      website: cp.website || (cleanName ? `https://www.${cleanName.toLowerCase().replace(/[^a-z0-9]/g, "")}.com` : void 0),
      phone: cleanPhone,
      email: cleanEmail,
    ...
    shipmentMetrics: {
      ...
      lastShipmentDate: sm.lastShipmentDate || (/* @__PURE__ */ new Date()).toISOString().split("T")[0],
      lastShipmentDetails: {
        date: ls.date || (/* @__PURE__ */ new Date()).toISOString().split("T")[0],
  ...
  return {
    id: `cnee-comm-${Math.floor(1e5 + Math.random() * 9e5)}`,
    rank: rankNum,
    consigneeName: cleanName,
    ...
    hsCodesList: item.commodities?.map((c) => c.hsCode).filter(Boolean) || ["CBP Tariff Chapter"],
    ...
    website: cp.website || (cleanName ? `https://www.${cleanName.toLowerCase().replace(/[^a-z0-9]/g, "")}.com` : void 0),
    phone: cleanPhone,
    keyContactName: contactObj.name || void 0,
    keyContactTitle: contactObj.title || void 0,
    keyContactEmailPhone: contactObj.emailOrPhone || void 0,
    fullProfile
  };
}
function generateSupplementalConsignees(queryTerm, hsCode, minTeus, originFilter, portFilter, cTpatOnly, countNeeded, existingNamesLower) {
  const INDUSTRY_PRESETS = {
    ...
    furniture: [
      ...
      { name: "Wayfair LLC", address: "4 Copley Pl, Boston, MA 02116", teus: 22e3, supplier: "Kuka Home Vietnam", hs: "9401.61" },
      { name: "IKEA Supply AG (USA)", address: "420 Alan Wood Rd, Conshohocken, PA 19428", teus: 34e3, supplier: "IKEA Swedwood Europe/Asia", hs: "9403.20" },
...
app.post("/api/profile-consignee", async (req, res) => {
  ...
  try {
    ...
    const isQuotaError = error?.status === 429 || errMsg.includes("RESOURCE_EXHAUSTED") || errMsg.includes("429") || errMsg.includes("quota");
    if (isQuotaError) {
      return res.status(429).json({
        error: "Gemini API rate limit or quota reached. Please wait 15-30 seconds before retrying your search or batch processing."
      });
    }
    res.status(500).json({
